...
.controller('maintenanceCtrl', [ '$scope', '$rootScope', '$window', '$sce', '$http', '$translate', 'sharedProperties', 'redirectService', 'applicationStatusService', function($scope, $rootScope, $window, $sce, $http, $translate, sharedProperties, redirectService, applicationStatusService) {
		...
		angular.forEach(adminIPOBook, function(item) {
			...
			//Day Count			
			if ($scope.isIPOClosed(ipo['subCloseTime'],ipo['offerCloseTime'])){
										
				}
				else if ($scope.isIPOComing(ipo['subStartTime'])) {
					
				}
				else if ($scope.isIPOSubEndIn(ipo['subStartTime'],item['subCloseTime'])) {
					
				}
				else if ($scope.isIPOSubEnd(ipo['subStartTime'],item['subCloseTime'],ipo['offerCloseTime'])) {
								
				}	
															
			//last modify
			if(item['approveUserId'] != null && item['approveUserId'] != undefined && item['approveUserId'] != "" ) {			
			ipo['lastModifiedUser'] = item['approveUserId'];
			ipo['lastModifiedDate'] = item['approveTime'];
			ipo['fmtLastModifiedDate'] = $scope.formattingDate(item['approveTime']);
			ipo['lastModifiedTime'] = $scope.formattingDateTime(item['approveTime']);
			}
			else {
			ipo['lastModifiedUser'] = item['createUserId'];
			ipo['fmtLastModifiedDate'] = $scope.formattingDate(item['createTime']);
			ipo['lastModifiedTime'] = $scope.formattingDateTime(item['createTime']);
			ipo['lastModifiedDate'] = item['approveTime'];
			}
	...
	$scope.displayError = function(errorCode) {
		if (errorCode) {
			...
			if (!errorCode.startsWith(IPO_ERROR_CODE_PREFIX)) {
				return redirectService.toIndex();							
			}
		}
		else {
			$scope.displayDefaultError();
	...
	$scope.usePrice = function(finalOfferPrice, status) {
		if($scope.checkNull(finalOfferPrice) && status == "Closed" )
		return true;
		else {
		return false;
		}
	...
	//sort button
	$scope.sortType     = "lastModifiedDate"; 
    ...
    $scope.sortBtnActive = function(value) {
		...
		if (value == "lastModTimeBtn"){	
			...
			$scope.sortType = 'lastModifiedUser'; 
			$scope.sortReverse = !$scope.sortReverse;
		}
	...
	$scope.showUser = function(adminIPO){
		...
		for (var user in $scope.filterUserId) {	
				if(adminIPO.lastModifiedUser === $scope.filterUserId[user] ) {
				filter = true;
				}		   
		   }			   
		return filter;
	...
	$scope.dateTo = '20170101';
	$scope.myselfCheckbox = 'N';
	
					
	$scope.descOption= 'Mod';
	$scope.descDay = 30;
	$scope.descDayFrom = '20170101';
	$scope.descDayTo = '20170101';
	$scope.descMyself = 'N';
	...
	$scope.onClockMyselfBox = function() {
